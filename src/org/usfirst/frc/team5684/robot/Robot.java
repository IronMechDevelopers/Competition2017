...
public class Robot extends SampleRobot {
	...
	public Robot() {
		...
		encoder = new Encoder(0,1,false,EncodingType.k1X); ///have to find right scaling factor (EncoderType)
	}
	...
	public void robotInit() {
		...
		System.out.println("robotInit: gyro.getAngle = " + myGyro.getAngle());
	...
	public void autonomous() {
		...
		if (true == SmartDashboard.getBoolean("DB/Button 0") && false == SmartDashboard.getBoolean("DB/Button 1")) {
			...
			cheval_de_frise();
		} else if (false == SmartDashboard.getBoolean("DB/Button 0") && true == SmartDashboard.getBoolean("DB/Button 1")) {
	...
	public void low_bar() {
		...
		double driveForTime=3500.0;  // This is the time to drive AFTER the arm is down
		double driveBeforeArm=0.0; //This is the time to drive BEFORE the arm is down

		double drivingScalingFactor = 1.2;  // this is to affect the speed and distance after the arm is down

		driveBeforeArm = 750.0;  // Number of loops before arm goes down

		driveForTime = driveForTime - driveBeforeArm;  // we need to drive forward for a bit before
		// the arm comes down.  this way, we can tweak
		// the value and adjust the total distance traveled
		driveForTime = driveForTime/drivingScalingFactor;  // this adjusts the loop - speed is adjusted below
		...
		while (!(armTimer.hasPeriodPassed(2.7))) //number passed in is in seconds        	
		{
		...
		while (drivingLoops < driveBeforeArm ) //|| !(armTimer.hasPeriodPassed(15)))
		{
			int encoder_count = 0;

			turningValue =  -myGyro.getAngle()*Kp;    // THIS WILL USE THE GYRO INPUT TO DETERMINE TURNING ANGLE

			myRobot.drive(-0.22, turningValue);    // THIS IS GOING FORWARDS (COLLECTOR IN FRONT)
			...
			{
				System.out.println("Distance: " + encoder.getDistance()+ " Count: " + encoder.get() + " Raw: " + encoder.getRaw());
		...
		double driveSpeed = 0.22 * drivingScalingFactor;  // to change speed, change scaling factor above
	...
	public void cheval_de_frise() {
		...
		myGyro.setSensitivity(voltsPerDegreePerSecond);
		myGyro.calibrate(); //Added by SRH at 15:11
		double driveForTime=0.0;  // This is the time to drive AFTER the arm is down
		double driveBeforeArm=0.0; //This is the time to drive BEFORE the arm is down

		//SCOTT - This drivingScalingFactor might need to be adjusted to affect the
		...
		while (!(armTimer.hasPeriodPassed(0.7))) //number passed in is in seconds        	
		{
			arm.setInverted(true);
		...
		while (!(armTimer.hasPeriodPassed(2.2))) //number passed in is in seconds        	
		{
			arm.setInverted(true);
		...
		double driveSpeed = 0.22 * drivingScalingFactor *2;  // to change speed, change scaling factor above

		while (isAutonomous() ) //|| !(armTimer.hasPeriodPassed(15)))
		{
			turningValue =  -myGyro.getAngle()*Kp;    // THIS WILL USE THE GYRO INPUT TO DETERMINE TURNING ANGLE
			//turningValue = 0.0;							// COMMENT OUT THIS LINE IF USING THE GYRO AND UNCOMMENT LINE ABOVE


			if (drivingLoops < driveForTime) {

				myRobot.drive(-driveSpeed, turningValue);    // needs to go a little faster
	...
	public void portcullis() {
		...
		while (!(armTimer.hasPeriodPassed(2.0))) //number passed in is in seconds        	
		{
